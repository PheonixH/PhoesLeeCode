...
import leetcode.specialDataStruct.Worker;

import java.util.*;
import java.util.function.Function;
import java.util.stream.Collectors;

import leetcode.specialDataStruct.*;
...
//    }
    ...
    */
    ...
    public leetcode.dataStruct.Node cloneGraph(leetcode.dataStruct.Node node) {
        if (node != null) {
            Set<leetcode.dataStruct.Node> set = new HashSet<>();
            leetcode.dataStruct.Node res = new leetcode.dataStruct.Node();
    ...
    public void cloneGraph(leetcode.dataStruct.Node node, leetcode.dataStruct.Node newNode, Set<leetcode.dataStruct.Node> visit) {
        if (node.neighbors != null) {
            if (newNode.neighbors == null) {
                newNode.neighbors = new ArrayList<>();
            }
            for (leetcode.dataStruct.Node n : node.neighbors) {
                if (visit.add(n)) {
                    leetcode.dataStruct.Node newN = new leetcode.dataStruct.Node();
                    newN.val = n.val;
...
//        boolean isSuccess = false;
    ...
    public int findLeastNumOfUniqueInts00(int[] arr, int k) {
        int len = arr.length;
    ...
    public int minDays0(int[] bloomDay, int m, int k) {
...
//            }
    ...
    public int[] sortedSquares00(int[] arr) {
...
//        return Math.max(tmp, tmp2);
     ...
     * @return 链表是否有环
     */
    public boolean hasCycle7(ListNode head) {
        if (head == null) {
            return false;
        }
...
//    }
     ...
     * @return 结果
     */
    public int[] sortedSquares7(int[] A) {
        PriorityQueue<Integer> priorityQueue = new PriorityQueue<>();
        for (int a : A) {
     ...
     * @return 几种摆法
     */
    public int totalNQueens7(int n) {
    ...
    public List<Integer> partitionLabels7(String S) {
...
//        int[] array = list.stream().mapToInt(i->i).toArray();
    ...
    public List<String> watchedVideosByFriends7(List<List<String>> watchedVideos, int[][] friends, int id, int level) {
        Queue<Integer> queue = new LinkedList<>();
    ...
    public int[] smallerNumbersThanCurrent7(int[] nums) {
        // 统计出现频率 frequency
        int[] freq = new int[101]; // 索引即数值
        for (int num : nums) {
            freq[num]++;
...
//        });
     ...
     */
    public int[][] kClosest7(int[][] points, int K) {
        if (K >= points.length) {
            return points;
        }
     ...
     * @return 按奇偶排序数组
     */
    public int[] sortArrayByParityII7(int[] A) {
        int n = A.length;
        int a = 0, b = 1;
        while (a < n && b < n) {
            while (a < n && A[a] % 2 == 0) {
     ...
     */
    public ListNode oddEvenList7(ListNode head) {
        ListNode one = head;
        if (one == null) {
            return head;
        }
...
//            int[] tmp = priorityQueue.poll();
     ...
     */
    public int canCompleteCircuit7(int[] gas, int[] cost) {
        int n = gas.length;
        for (int i = 0; i < n; i++) {
            if (gas[i] - cost[i] < 0) {
                continue;
    ...
    private boolean isBalancedFlag2 = true;
...
//                }
     ...
     * 内存消耗：35.6 MB, 在所有 Java 提交中击败了87.37% 的用户
     * @param stones 每块石头的重量
     * @return 最后一块石头的重量
     */
